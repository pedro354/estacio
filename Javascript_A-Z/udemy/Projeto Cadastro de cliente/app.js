...
const mensagemCartao = document.querySelector("#mensagemCartao")
const {nome, celular, cpf, dataNascimento, email, senha, confirmaSenha, numCartao, codigoSeg, dataValidade} = form;
...
function validaEmail(){
    const regex = /^[a-zA-Z0-9_]+@[a-zA-Z0-9_]+\.[a-zA-Z0-9_]+/; // /\w+@\w+\.w+/;
...
function validaSenha(){
    ...
    if(regex.test(senha.value)){
        ...
        throw new Error("Senha inválida, senha deve conter pelo menos uma Letra, Número ou Caracter Especial");
    }
}
...
/*
function validaCartao(){
    const regex = /^(?<C1>^5[1-5][0-9]{14}) (?<C2>4[0-9]{12}(?:[0-9]{3})) (?<C3>[0-9]{4}) (?<C4>[0-9]{4})$/;
    const cartao = regex.exec(numCartao.value);
    if(cartao.groups.C1){
        mensagemCartao.innerHTML == "Mastercard";
        console.log("Você escolheu mastercard")
    }
    else if (cartao.groups.C2 == '0000'){
        mensagemCartao.innerHTML == "Visa";
        console.log("Você escolheu Visa")
